...
%union {
  ...
  modal_formula_ast* _modal_formula_ast; 
  fairness_expression* _fairness_expression;
  pair<modal_formula*, modal_formula*>* dual_modal_formula;
  pair<modal_formula_ast*, modal_formula_ast*>* dual_modal_formula_ast;
  pair<string*, modal_formula*>* ID_modal_formula;
  pair<string*, modal_formula_ast*>* ID_modal_formula_ast;
  pair<pair<string*, string*>*, modal_formula*>* ID_ID_modal_formula;
  pair<pair<string*, string*>*, modal_formula_ast*>* ID_ID_modal_formula_ast;
...
%token AGENT STRTYPE ACTIONS PROTOCOL EVOLUTION END LB RB LCB RCB LT GT IF EQ AND
...
%token FORMULAE FORMULAE_AST IMPLIES AG EG AX EX AF EF A E UNTIL RELEASE K GK GCK O DK
...
%left AND BITAND
%left NOT BITNOT
%left EX AX EF AF EG AG E A UNTIL RELEASE
...
%type <sval> strtypedef 
%type <sval> strtypename
...
%type <_modal_formula_ast> formula_ast

%type <_fairness_expression> fformula

%type <ival> strtypeprefix
%type <ival> strtypesuffix
...
%type <ival> forprefix
%type <ival> forsuffix
%type <ival> forastprefix
%type <ival> forastsuffix
...
%type <ival> agsuffix
%type <sval> atlprefix
%type <sval> atlstrprefix 
%type <dual_modal_formula> untilprefix
%type <dual_modal_formula> releaseprefix
%type <dual_modal_formula_ast> untilastprefix 
%type <dual_modal_formula_ast> releaseastprefix 

%type <ID_modal_formula> epistemicprefix
%type <ID_modal_formula> gepistemicprefix

%type <ID_modal_formula_ast> epistemicprefix_ast
%type <ID_modal_formula_ast> gepistemicprefix_ast
...
;
 is: environment agents evaluation istates groups fairness formulae formulae_ast
 | environment agents evaluation istates groups fairness formulae_ast
 | environment agents evaluation istates groups fairness formulae
 | environment agents evaluation istates groups formulae formulae_ast
 | environment agents evaluation istates groups formulae_ast
 | environment agents evaluation istates groups formulae
 | environment agents evaluation istates fairness formulae formulae_ast
 | environment agents evaluation istates fairness formulae_ast
 | environment agents evaluation istates fairness formulae
 | environment agents evaluation istates formulae formulae_ast
 | environment agents evaluation istates formulae_ast
 | environment agents evaluation istates formulae
 | agents evaluation istates groups fairness formulae formulae_ast {
 ...
 | agents evaluation istates groups fairness formulae_ast {
   basic_agent *env = new basic_agent((*string_table)[0], new string("IR"), NULL, new map<string, basictype *>, new vector<basictype *>,
					  new map<string, basictype *>, new vector<basictype *>, NULL, new set<string>, 
					  new vector<protocol_line*>, new vector<evolution_line *>);
   (*is_agents)["Environment"] = env;
 ...
 | agents evaluation istates groups formulae formulae_ast  {
   basic_agent *env = new basic_agent((*string_table)[0], new string("IR"), NULL, new map<string, basictype *>, new vector<basictype *>, 
					  new map<string, basictype *>, new vector<basictype *>, NULL, new set<string>, 
					  new vector<protocol_line*>, new vector<evolution_line *>);
   (*is_agents)["Environment"] = env;
 ...
 | agents evaluation istates groups formulae_ast  {
   basic_agent *env = new basic_agent((*string_table)[0], new string("IR"), NULL, new map<string, basictype *>, new vector<basictype *>, 
					  new map<string, basictype *>, new vector<basictype *>, NULL, new set<string>, 
					  new vector<protocol_line*>, new vector<evolution_line *>);
   (*is_agents)["Environment"] = env;
 ...
 | agents evaluation istates fairness formulae formulae_ast	{
   basic_agent *env = new basic_agent((*string_table)[0], new string("IR"), NULL, new map<string, basictype *>, new vector<basictype *>, 
					  new map<string, basictype *>, new vector<basictype *>, NULL, new set<string>, 
					  new vector<protocol_line*>, new vector<evolution_line *>);
   (*is_agents)["Environment"] = env;
 ...
 | agents evaluation istates fairness formulae_ast	{
   basic_agent *env = new basic_agent((*string_table)[0], new string("IR"), NULL, new map<string, basictype *>, new vector<basictype *>, 
					  new map<string, basictype *>, new vector<basictype *>, NULL, new set<string>, 
					  new vector<protocol_line*>, new vector<evolution_line *>);
   (*is_agents)["Environment"] = env;
 ...
 | agents evaluation istates formulae formulae_ast {
   basic_agent *env = new basic_agent((*string_table)[0], new string("IR"), NULL, new map<string, basictype *>, new vector<basictype *>, 
					  new map<string, basictype *>, new vector<basictype *>, NULL, new set<string>, 
					  new vector<protocol_line*>, new vector<evolution_line *>);
   (*is_agents)["Environment"] = env;
 ...
 | agents evaluation istates formulae_ast {
   basic_agent *env = new basic_agent((*string_table)[0], new string("IR"), NULL, new map<string, basictype *>, new vector<basictype *>, 
					  new map<string, basictype *>, new vector<basictype *>, NULL, new set<string>, 
					  new vector<protocol_line*>, new vector<evolution_line *>);
   (*is_agents)["Environment"] = env;
...
environment: envprefix strtypedef obsvardef envardef enreddef enactiondef enprotdef envevdef envsuffix {
  if($1==1 && $2!=NULL && $3!=NULL && $4!=NULL && $6!=NULL && $7!=NULL && $8!=NULL && $9==1) { // redstates is NULL if no red state at all
    basic_agent *env = new basic_agent((*string_table)[0], $2,NULL, $3->first, $3->second, $4->first, $4->second, $5, $6, $7, $8);
    (*is_agents)["Environment"] = env;
    agents->push_back(env);
  }
 }
| envprefix strtypedef obsvardef envardef enactiondef enprotdef envevdef envsuffix {
  if($1==1 && $2!=NULL && $3!=NULL && $4!=NULL && $5!=NULL && $6!=NULL && $7!=NULL && $8==1) { // redstates is NULL if no red state at all
    basic_agent *env = new basic_agent((*string_table)[0], $2, NULL, $3->first, $3->second, $4->first, $4->second, NULL, $5, $6, $7);
    (*is_agents)["Environment"] = env;
    agents->push_back(env);
  }
 }
| envprefix strtypedef envardef enreddef enactiondef enprotdef envevdef envsuffix {
  if($1==1 && $2!=NULL && $3!=NULL && $5!=NULL && $6!=NULL && $7!=NULL && $8==1) { // redstates is NULL if no red state at all
    basic_agent *env = new basic_agent((*string_table)[0], $2, NULL, new map<string, basictype *>, new vector<basictype *>, $3->first, $3->second, $4, $5, $6, $7);
    (*is_agents)["Environment"] = env;
    agents->push_back(env);
  }
 }
| envprefix strtypedef envardef enactiondef enprotdef envevdef envsuffix {
  if($1==1 && $2!=NULL && $3!=NULL && $4!=NULL && $5!=NULL && $6!=NULL && $7==1) { // redstates is NULL if no red state at all
    basic_agent *env = new basic_agent((*string_table)[0],$2, NULL, new map<string, basictype *>, new vector<basictype *>, $3->first, $3->second, NULL, $4, $5, $6);
    (*is_agents)["Environment"] = env;
    agents->push_back(env);
  }
 }
| envprefix strtypedef obsvardef enreddef enactiondef enprotdef envevdef envsuffix {
  if($1==1 && $2!=NULL && $3!=NULL && $5!=NULL && $6!=NULL && $7!=NULL && $8==1) { // redstates is NULL if no red state at all
    basic_agent *env = new basic_agent((*string_table)[0], $2, NULL, $3->first, $3->second, new map<string, basictype *>, new vector<basictype *>, $4, $5, $6, $7);
    (*is_agents)["Environment"] = env;
    agents->push_back(env);
  }
 }
| envprefix strtypedef obsvardef enactiondef enprotdef envevdef envsuffix {
  if($1==1 && $2!=NULL && $3!=NULL && $4!=NULL && $5!=NULL && $6!=NULL && $7==1) { // redstates is NULL if no red state at all
    basic_agent *env = new basic_agent((*string_table)[0], $2, NULL, $3->first, $3->second, new map<string, basictype *>, new vector<basictype *>, NULL, $4, $5, $6);
    (*is_agents)["Environment"] = env;
    agents->push_back(env);
  }
 }
| envprefix strtypedef enreddef enactiondef enprotdef envevdef envsuffix {
  if($1==1 && $2!=NULL && $4!=NULL && $5!=NULL && $6!=NULL && $7==1) { // redstates is NULL if no red state at all
    basic_agent *env = new basic_agent((*string_table)[0],$2, NULL, new map<string, basictype *>, new vector<basictype *>, 
				       new map<string, basictype *>, new vector<basictype *>, $3, $4, $5, $6);
    (*is_agents)["Environment"] = env;
    agents->push_back(env);
  }
 }
| envprefix strtypedef enactiondef enprotdef envevdef envsuffix {
  if($1==1 && $2!=NULL && $3!=NULL && $4!=NULL && $5!=NULL && $6==1) { // redstates is NULL if no red state at all
    basic_agent *env = new basic_agent((*string_table)[0], $2, NULL, new map<string, basictype *>, new vector<basictype *>, 
				       new map<string, basictype *>, new vector<basictype *>, NULL, $3, $4, $5);
    (*is_agents)["Environment"] = env;
    agents->push_back(env);
  }
 }
...
/* definition of strategy type */
strtypeprefix: STRTYPE COLON {
  $$ = 1;
}
| error COLON {
  $$ = 0;
...
strtypedef: strtypeprefix strtypename strtypesuffix {
  if($1==1 && $3==1)
    $$ = $2;
  else
	$$ = NULL;
}
| strtypeprefix strtypesuffix {
	$$ = NULL ;
}
strtypename: ID SEMICOLON {
	if($1!=NULL) {
...
agent: agprefix ID strtypedef vardef reddef actiondef protdef evdef agsuffix {
  if($1==1 && $2!=NULL && $3!=NULL && $4!=NULL && 
     $6!=NULL && $7!=NULL && $8!=NULL && $9==1) {// redstates is NULL if no red state at all
    int i = search_string_table($2);
    if(i>=0) {
      $$ = new basic_agent((*string_table)[i],$3, NULL, NULL, NULL, $4->first, $4->second, $5, $6, $7, $8);
      free($2);
    } else {
      $$ = new basic_agent($2, $3, NULL, NULL, NULL, $4->first, $4->second, $5, $6, $7, $8);
      string_table->push_back($2);
    }
  } else
    $$ = NULL;
 } 
| agprefix ID strtypedef vardef actiondef protdef evdef agsuffix {
  if($1==1 && $2!=NULL && $3!=NULL && 
     $4!=NULL && $5!=NULL && $6!=NULL && $7!=NULL && $8==1) {// redstates is NULL if no red state at all
    int i = search_string_table($2);
    if(i>=0) {
      $$ = new basic_agent((*string_table)[i], $3, NULL, NULL, NULL, $4->first, $4->second, NULL, $5, $6, $7);
      free($2);
    } else {
      $$ = new basic_agent($2, $3, NULL, NULL, NULL, $4->first, $4->second, NULL, $5, $6, $7);
      string_table->push_back($2);
    }
  } else
    $$ = NULL;
 } 
| agprefix ID strtypedef lobsvarsdef vardef reddef actiondef protdef evdef agsuffix {
  if($1==1 && $2!=NULL && $3!=NULL && $5!=NULL && 
     $7!=NULL && $8!=NULL && $9!=NULL && $10==1) {// redstates is NULL if no red state at all
    int i = search_string_table($2);
    if(i>=0) {
      $$ = new basic_agent((*string_table)[i], $3, $4, NULL, NULL, $5->first, $5->second, $6, $7, $8, $9);
      free($2);
    } else {
      $$ = new basic_agent($2, $3,$4, NULL, NULL, $5->first, $5->second, $6, $7, $8, $9);
      string_table->push_back($2);
    }
  } else
    $$ = NULL;
 } 
| agprefix ID strtypedef lobsvarsdef vardef actiondef protdef evdef agsuffix {
  if($1==1 && $2!=NULL && $3!=NULL && 
     $5!=NULL && $6!=NULL && $7!=NULL && $8!=NULL && $9==1) {// redstates is NULL if no red state at all
    int i = search_string_table($2);
    if(i>=0) {
      $$ = new basic_agent((*string_table)[i],$3,$4, NULL, NULL, $5->first, $5->second, NULL, $6, $7, $8);
      free($2);
    } else {
      $$ = new basic_agent($2,$3, $4, NULL, NULL, $5->first, $5->second, NULL, $6, $7, $8);
      string_table->push_back($2);
    }
  } else
    $$ = NULL;
...
| error formula error formula error {
 ...
 releaseprefix: LB formula RELEASE formula RB{
  if($2!=NULL && $4!=NULL)
    $$ = new pair<modal_formula *, modal_formula *>($2, $4);
  else
    $$ = NULL;
 }
| error formula RELEASE formula RB {
  $$ = NULL;
  driver.syntax_error = true;
  USE($2);USE($4);
 }
...
| LB formula RELEASE formula error {
  $$ = NULL;
  driver.syntax_error = true;
  USE($2);USE($4);
 }
...
| error formula RELEASE formula error {
  $$ = NULL;
  driver.syntax_error = true;
  USE($2);USE($4);
 }
...
| A releaseprefix {
  if($2!=NULL) {
    $$ = new modal_formula(18, $2->first, $2->second);
    free($2);
  }
  else
    $$ = NULL;
...
| E releaseprefix {
  if($2!=NULL) {
    $$ = new modal_formula(19, $2->first, $2->second);
    free($2);
  }
  else
    $$ = NULL;
...
| error {
  ...
  formulae_ast: forastprefix formastlist forastsuffix;

  
 formastlist: formula_ast SEMICOLON { 
	if($1!=NULL) {
	  is_formulae_ast->push_back(*$1);
	}
   }
  | formula_ast error {
	driver.syntax_error = true;
  ...
  | formastlist formula_ast SEMICOLON { 
	if($2!=NULL) {
	  is_formulae_ast->push_back(*$2); 
	}
   }
  | formastlist formula_ast error {
	driver.syntax_error = true;
	USE($2);
   }
  | error {
  ...
  untilastprefix: LB formula_ast UNTIL formula_ast RB{
	if($2!=NULL && $4!=NULL)
	  $$ = new pair<modal_formula_ast *, modal_formula_ast *>($2, $4);
	else
	  $$ = NULL;
   }
  | error formula_ast UNTIL formula_ast RB {
	$$ = NULL;
	driver.syntax_error = true;
	USE($2);USE($4);
   }
  | LB formula_ast error formula_ast RB {
	$$ = NULL;
	driver.syntax_error = true;
	USE($2);USE($4);
   }
  | LB formula_ast UNTIL formula_ast error {
	$$ = NULL;
	driver.syntax_error = true;
	USE($2);USE($4);
   }
  | error formula_ast error formula_ast RB {
	$$ = NULL;
	driver.syntax_error = true;
	USE($2);USE($4);
   }
  | error formula_ast UNTIL formula_ast error {
	$$ = NULL;
	driver.syntax_error = true;
	USE($2);USE($4);
   }
  | LB formula_ast error formula_ast error {
	$$ = NULL;
	driver.syntax_error = true;
	USE($2);USE($4);
   }
  | error formula_ast error formula_ast error {
	$$ = NULL;
	driver.syntax_error = true;
	USE($2);USE($4);
   };  
 releaseastprefix: LB formula_ast RELEASE formula_ast RB{
	if($2!=NULL && $4!=NULL)
	  $$ = new pair<modal_formula_ast *, modal_formula_ast *>($2, $4);
	else
	  $$ = NULL;
   }
  | error formula_ast RELEASE formula_ast RB {
	$$ = NULL;
	driver.syntax_error = true;
	USE($2);USE($4);
   }
  ...
  | LB formula_ast RELEASE formula_ast error {
	$$ = NULL;
	driver.syntax_error = true;
	USE($2);USE($4);
   }
  ...
  | error formula_ast RELEASE formula_ast error {
	$$ = NULL;
	driver.syntax_error = true;
	USE($2);USE($4);
   }
  ...
  epistemicprefix_ast: LB ID COMMA formula_ast RB{
	if($2!=NULL && $4!=NULL) {
	  int i = search_string_table($2);
	  if(i>=0) {
	    $$ = new pair<string *, modal_formula_ast *>((*string_table)[i], $4);
	    free($2);
	  } else {
	    $$ = new pair<string *, modal_formula_ast *>($2, $4);
	    string_table->push_back($2);
	  }
	} else
	  $$ = NULL;
   }
  | error ID COMMA formula_ast RB {
	 $$ = NULL;
	 driver.syntax_error = true;
	 USE($2);
	 USE($4);
	}
   | LB ID error formula_ast RB {
	 $$ = NULL;
	 driver.syntax_error = true;
	 USE($2);
	 USE($4);
    }
   | LB ID COMMA formula_ast error {
	 $$ = NULL;
	 driver.syntax_error = true;
	 USE($2);
	 USE($4);
	}
   | error ID error formula_ast RB {
	 $$ = NULL;
	 driver.syntax_error = true;
	 USE($2);
	 USE($4);
	}
   | error ID COMMA formula_ast error {
	 $$ = NULL;
	 driver.syntax_error = true;
	 USE($2);
	 USE($4);
	}
   | LB ID error formula_ast error {
	 $$ = NULL;
	 driver.syntax_error = true;
	 USE($2);
	 USE($4);
	}
   | error ID error formula_ast error {
	 $$ = NULL;
	 driver.syntax_error = true;
	 USE($2);
	 USE($4);
	}
   | LB ENVIRONMENT COMMA formula_ast RB{
	 if($4!=NULL)
	   $$ = new pair<string *, modal_formula_ast *>((*string_table)[0], $4);
	 else
	   $$ = NULL;
	}
   | error ENVIRONMENT COMMA formula_ast RB {
	 $$ = NULL;
	 driver.syntax_error = true;
	 USE($4);
	}
   | LB ENVIRONMENT error formula_ast RB {
	 $$ = NULL;
	 driver.syntax_error = true;
	 USE($4);
	}
   | LB ENVIRONMENT COMMA formula_ast error {
	 $$ = NULL;
	 driver.syntax_error = true;
	 USE($4);
	}
   | error ENVIRONMENT error formula_ast RB {
	 $$ = NULL;
	 driver.syntax_error = true;
	 USE($4);
	}
   | error ENVIRONMENT COMMA formula_ast error {
	 $$ = NULL;
	 driver.syntax_error = true;
	 USE($4);
	}
   | LB ENVIRONMENT error formula_ast error {
	 $$ = NULL;
	 driver.syntax_error = true;
	 USE($4);
	}
   | error ENVIRONMENT error formula_ast error {
	 $$ = NULL;
	 driver.syntax_error = true;
	 USE($4);
	};
  gepistemicprefix_ast: LB ID COMMA formula_ast RB{
	if($2!=NULL && $4!=NULL) {
	  int i = search_string_table($2);
	  if(i>=0) {
	    $$ = new pair<string *, modal_formula_ast *>((*string_table)[i], $4);
  ...
  formula_ast: LB formula_ast RB { 
	$$ = $2;
   }
  | error formula_ast RB { 
	$$ = NULL;
	driver.syntax_error = true;
	USE($2);
   }
  | LB formula_ast error { 
	$$ = NULL;
	driver.syntax_error = true;
	USE($2);
   }
  | error formula_ast error { 
	$$ = NULL;
	driver.syntax_error = true;
	USE($2);
   }
  | formula_ast AND formula_ast {
	if($1!=NULL && $3!=NULL)
	  $$ = new modal_formula_ast(1, $1, $3);
	else
	  $$ = NULL;
   }
  | formula_ast OR formula_ast {
	if($1!=NULL && $3!=NULL)
	  $$ = new modal_formula_ast(2, $1, $3);
	else
	  $$ = NULL;
   }
  | formula_ast error formula_ast {
	$$ = NULL;
	driver.syntax_error = true;
	USE($1);USE($3);
   }
  ...
  | formula_ast IMPLIES formula_ast {
	if($1!=NULL && $3!=NULL)
	  $$ = new modal_formula_ast(4, $1, $3);
	else
	  $$ = NULL;
   }
  | X formula_ast {
	if($2!=NULL)
	  $$ = new modal_formula_ast(5, $2);
	else
	  $$ = NULL;
   }
  | F formula_ast {
	if($2!=NULL)
	  $$ = new modal_formula_ast(6, $2);
	else
	  $$ = NULL;
   }
  | G formula_ast { 
	if($2!=NULL)
	  $$ = new modal_formula_ast(7, $2);
	else
	  $$ = NULL;
   }
  | untilastprefix {
	if($1!=NULL) {
	  $$ = new modal_formula_ast(8, $1->first, $1->second);
	  free($1);
	}
	else
	  $$ = NULL;
  ...
  | releaseastprefix {
	if($1!=NULL) {
	  $$ = new modal_formula_ast(9, $1->first, $1->second);
	  free($1);
	}
	else
	  $$ = NULL;
   }
  | K epistemicprefix_ast {
	if($2!=NULL) {
	  atomic_proposition *ap = new atomic_proposition($2->first);
	  modal_formula_ast *a = new modal_formula_ast(ap);
	  $$ = new modal_formula_ast(10, a, $2->second);
	  free($2);
	} else
	  $$ = NULL;
   }
  | GK gepistemicprefix_ast {
    // Knowledge in a group (everybody knows)
	if($2!=NULL) {
      atomic_proposition *ap = new atomic_proposition($2->first);
      modal_formula_ast *a = new modal_formula_ast(ap);
      $$ = new modal_formula_ast(11, a, $2->second);
      free($2);
    } else
      $$ = NULL;
   }
  | GCK gepistemicprefix_ast {
    // Common knowledge in a group
	if($2!=NULL) {
      atomic_proposition *ap = new atomic_proposition($2->first);
      modal_formula_ast *a = new modal_formula_ast(ap);
      $$ = new modal_formula_ast(12, a, $2->second);
      free($2);
    } else
      $$ = NULL;
   }
  | DK gepistemicprefix_ast {
	// Distributed knowledge in a group
    if($2!=NULL) {
      atomic_proposition *ap = new atomic_proposition($2->first);
      modal_formula_ast *a = new modal_formula_ast(ap);
      $$ = new modal_formula_ast(13, a, $2->second);
      free($2);
    } else
      $$ = NULL;
   }
  | error epistemicprefix_ast { 
    $$ = NULL;
    driver.syntax_error = true;
   }
  | error gepistemicprefix_ast { 
    $$ = NULL;
    driver.syntax_error = true;
   }
  | atlstrprefix formula_ast {
    if($1!=NULL && $2!=NULL) {
	  atomic_proposition *ap = new atomic_proposition($1);
	  modal_formula_ast *a = new modal_formula_ast(ap);
	  $$ = new modal_formula_ast(14, a, $2);
	} else {
	  if($2!= NULL)
	  	free($2);
	  if($1!=NULL)
