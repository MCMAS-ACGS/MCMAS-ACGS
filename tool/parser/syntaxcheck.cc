...
global_consistency_check()
{
  ...
  pair < string, map< string, basictype * > >*a = new pair < string, map< string, basictype * > >(is_environment->get_name(), en_vars);
  global_vars.insert(*a);
  for (unsigned int i = 1; i < agents->size(); i++) {
    a=new pair < string,map< string,basictype * > >((*agents)[i]->get_name(), *((*agents)[i]->get_vars()));
    global_vars.insert(*a);
  }
  /*
  ...
  for (unsigned int t = 0; t < agents->size(); t++) {
    ...
    string agent_strategytype = agent->get_strategy_type();
    if (options["verbosity"] > 4){
		cout << "Check agent " << agent_name << " ...... " << endl;
		cout << "Checking the strategy type ";cout << agent_strategytype << " ...... " << endl;
	}
	if((agent_strategytype!="IR")&&(agent_strategytype!="Ir")&&(agent_strategytype!="ior")&&(agent_strategytype!="isr")){
	  cout<<agent_name<<"'s strategy type Error !!!"<<endl;
	  is_consistent = false;
	}
    ...
    for (vector< protocol_line * >::iterator i = ag_prot->begin();
      ...
      if (options["verbosity"] > 4)
        cout << "    NO. " << ++count << " ... " << endl;
	  bool_expression * tmp_con=(*i)->get_condition();
	  string tmp_con_string=tmp_con->to_string();
  ...
  for (map< string, set< string > >::iterator i = is_groups->begin();
	...
	if(((*i).second.size()==1)&&(*((*i).second.begin())=="Empty"))
	  continue;
   ...
   */
  if (options["verbosity"] > 4)
    cout << endl << "Checking formula_ast definition ..." << endl;
  count = 0;
  for (vector< modal_formula_ast >::iterator i = is_formulae_ast->begin();
       i != is_formulae_ast->end(); i++) {
    if (options["verbosity"] > 4)
      cout << "  formula_ast " << ++count << " ... " << endl;
    if (!(*i).check_atomic_proposition(0))
      is_consistent = false;
  }

  cout<<"global_consistency_check....over!!!"<<endl;
