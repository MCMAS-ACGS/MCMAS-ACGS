...
string
basic_agent::get_strategy_type(){
	if (strategy_type == NULL) {
    cout << " error: the strategy of type in agent is undefined!" << endl;
    exit(1);
  } else
    return *strategy_type;
}
string *
basic_agent::get_strategy_type_point(){
  return strategy_type;
}
...
vector< vector<bool> *> *
basic_agent::generate_basictype_bdd_value(int index_begin, int index_end, basictype * b_t)
{
  vector < vector<bool> * > *vec_vec = new vector< vector<bool> *>;
  vector <bool> base((index_end-index_begin+1),false);
  
  int bt_elementsnum=0;
  unsigned char value_type=b_t->get_type();
  if (value_type == 1) { 
  	  bt_elementsnum=2;
  }
  else if(value_type == 2){ 
  	  rangedint * b_t_rangedint=dynamic_cast<rangedint *>(b_t);
	  bt_elementsnum=b_t_rangedint->get_length();
  }
  else {
      enumerate * b_t_enumerate=dynamic_cast<enumerate *>(b_t);
	  bt_elementsnum=b_t_enumerate->get_number_of_elements();
  }

  for (int i = 0; i < bt_elementsnum; i++)
  {
    vector <bool> * tmp = new vector<bool>(base);
	vec_vec->push_back(tmp);
  	for(int j=index_end;j>=index_begin;j--){
		if(base[j])
			base[j]=false;
		else{
			base[j]=true;
...
vector<BDD>* 
basic_agent::get_state_vec(BDD others_condition, bdd_parameters * para){
  vector<BDD>* vec = new vector<BDD>;
  BDD tmp_condition = project_local_state(&others_condition, para->bddmgr, para->v);
  vector<BDD> my_v = get_localstate_BDD_vector(para);
  
  map<string, map<string, vector<BDD*>* >* >* int_vars = integer_variable_BDDs(para->bddmgr, &my_v);
  while(tmp_condition!=para->bddmgr->bddZero()){
	BDD curinitstate = tmp_condition.PickOneMinterm(my_v);
	curinitstate = complete_integer_BDDs(para->bddmgr, &my_v, curinitstate, int_vars);
	tmp_condition = tmp_condition - curinitstate;	
	bool valid = is_valid_local_state(curinitstate, *para->v);
		
	if (valid){ 
	  vec->push_back(curinitstate);
    }
  }  
  return vec;
}
...
void
basic_agent::get_other_protocol(bdd_parameters* para, vector<BDD>* all_other_condition_vec, set<string>* act_k, int k, vector<BDD>* all_case_prot, BDD oneline_tmp){ 
  BDD oneline_bef=oneline_tmp;
  if(k < all_other_condition_vec->size()-1){
  	BDD con_k_tmp = (*all_other_condition_vec)[k];
	for(set< string >::iterator i = act_k->begin(); i != act_k->end();i++) {
	  BDD oneline_after = oneline_bef + con_k_tmp*encode_action(para,*i);
	  this->get_other_protocol(para, all_other_condition_vec, act_k, k+1, all_case_prot, oneline_after);
	}
  }
  else{
  	BDD con_k_tmp = (*all_other_condition_vec)[k];
	for(set< string >::iterator i = act_k->begin(); i != act_k->end(); i++) {
	  BDD oneline_after = oneline_bef + con_k_tmp * encode_action(para,*i);
	  all_case_prot->push_back(oneline_after);
	}
  }
}

void 
basic_agent::encode_protocol_Ir(bdd_parameters* para, vector<pair<BDD, set<string>*>>* my_prot_Ir, unsigned int k, vector<BDD*>* all_case_prot, BDD oneline_temp){
  BDD oneline_bef = oneline_temp;
  set< string > *act_k=(*my_prot_Ir)[k].second;
  BDD con_k_tmp = (*my_prot_Ir)[k].first; 
  if(k < my_prot_Ir->size()-1){
	for(set< string >::iterator i = act_k->begin(); i != act_k->end();i++) {
	  BDD oneline_after = oneline_bef + con_k_tmp * encode_action(para,*i);
	  this->encode_protocol_Ir(para, my_prot_Ir, k+1, all_case_prot, oneline_after);
	}
  }
  else {
	for(set< string >::iterator i = act_k->begin(); i != act_k->end();i++) {
	  BDD oneline_after = oneline_bef + con_k_tmp * encode_action(para,*i);
	  all_case_prot->push_back(new BDD(oneline_after));
	}		
  }  
}

vector<BDD> basic_agent::get_state_vec_Ir(BDD con_k_tmp, bdd_parameters* para){
  vector<BDD> vec;
  map<string, map<string, vector<BDD*>* >* >* int_vars = integer_variable_BDDs(para->bddmgr, para->v);
  BDD bddzero = para->bddmgr->bddZero();
  while(con_k_tmp!=para->bddmgr->bddZero()){
	BDD curinitstate = con_k_tmp.PickOneMinterm(*para->v);
	curinitstate = complete_integer_BDDs(para->bddmgr, para->v, curinitstate, int_vars);    
	con_k_tmp = con_k_tmp - curinitstate;	
	bool if_reach = ((curinitstate*(*para->reach))!=bddzero);
	bool valid = is_valid_local_state_Ir(curinitstate, *para->v);	
	if (valid&&if_reach){ 
	  vec.push_back(curinitstate);
	}
  }  
...
vector<BDD*>* 
basic_agent::get_abs_protocol_Ir(bdd_parameters * para){
  if (protocol->size() == 0){
  ...
  vector<pair<BDD, set<string>*>>* my_prot_Ir=new vector<pair<BDD, set<string>*>>;
  if(protocol->back()->get_condition()->is_other_branch()){	
  	BDD* other_condition = new BDD(*para->reach);
	for(unsigned k=0;k<protocol->size()-1;k++){
	  bool_expression *con_k=(*protocol)[k]->get_condition();
	  set< string > *act_k=(*protocol)[k]->get_actions();
	  BDD con_k_tmp = con_k->encode_bdd_flat(para, para->bddmgr->bddOne());
	  *other_condition=(*other_condition)*(!con_k_tmp);

	  if(act_k->size()==1){
	    pair<BDD, set<string>*> p(con_k_tmp,act_k);
	    my_prot_Ir->push_back(p);
	  } else{
	  	vector<BDD> con_k_tmp_Ir = get_state_vec_Ir(con_k_tmp,para);
	    for(unsigned int i=0;i<con_k_tmp_Ir.size();i++){
	      pair<BDD, set<string>*> p(con_k_tmp_Ir[i],act_k);
	      my_prot_Ir->push_back(p);
	    }	
	  }	   
    }
	...
	//for other branch
	set< string > *act_k = (*protocol)[protocol->size()-1]->get_actions();
	if(act_k->size()==1){
	  pair<BDD, set<string>*> p(*other_condition,act_k);
	  my_prot_Ir->push_back(p);
	} else{
	  vector<BDD> all_other_condition_vec_Ir = get_state_vec_Ir(*other_condition,para);
	  for(unsigned int i=0;i<all_other_condition_vec_Ir.size();i++){
		pair<BDD, set<string>*> p((all_other_condition_vec_Ir)[i],act_k);
		my_prot_Ir->push_back(p);
	  }
	}    	 
  }
  else{
	for(unsigned k=0;k<protocol->size();k++){
	  bool_expression *con_k=(*protocol)[k]->get_condition();
	  set< string > *act_k=(*protocol)[k]->get_actions();
	  BDD con_k_tmp = con_k->encode_bdd_flat(para, para->bddmgr->bddOne());
	  if(act_k->size()==1){
  ...
  BDD* other_condition = new BDD(*para->reach);
  encode_protocol_Ir(para, my_prot_Ir, 0, uni_prot, oneline_temp);
  return uni_prot;
}


void basic_agent::encode_protocol_ir(bdd_parameters * para, unsigned int k, vector<BDD*>* all_case_prot, BDD oneline_tmp, BDD* other_condition)
{
  if(protocol->back()->get_condition()->is_other_branch()){
    bool_expression *con_k=(*protocol)[k]->get_condition();
    set< string > *act_k=(*protocol)[k]->get_actions();
    BDD oneline_bef=oneline_tmp;
    if(k < protocol->size()-1){
  	  BDD con_k_tmp = con_k->encode_bdd_flat(para, para->bddmgr->bddOne());
  	  *other_condition=(*other_condition)*(!con_k_tmp);
	  for(set< string >::iterator i = act_k->begin(); i != act_k->end();i++) {
		BDD oneline_after = oneline_bef + con_k_tmp*encode_action(para,*i);
		this->encode_protocol_ir(para, k+1, all_case_prot,oneline_after,other_condition);
	  }
    }
    else {
	  if(act_k->size()==1){
		BDD con_k_tmp = *other_condition;
		for(set< string >::iterator i = act_k->begin(); i != act_k->end(); i++) {
		  BDD oneline_after = oneline_bef + con_k_tmp * encode_action(para,*i);
		  all_case_prot->push_back(new BDD(oneline_after));
		}
		...
		vector<BDD>* all_other_condition_vec = get_state_vec(*other_condition,para);
	    BDD oneline_tmp = para->bddmgr->bddZero();
	    vector<BDD>* all_other_prot = new vector<BDD>;
	  	get_other_protocol(para, all_other_condition_vec, act_k, 0, all_other_prot, oneline_tmp);
		for(vector<BDD>::iterator i = all_other_prot->begin(); i != all_other_prot->end(); i++) {
		  BDD oneline_after = oneline_bef + *(i);
		  all_case_prot->push_back(new BDD(oneline_after));
		}		
	  }
    }
    ...
    BDD oneline_bef=oneline_tmp;
    if(k < protocol->size()-1){
  	  BDD con_k_tmp = con_k->encode_bdd_flat(para, para->bddmgr->bddOne());//protocol第k行的condition的BDD
	  for(set< string >::iterator i = act_k->begin(); i != act_k->end();i++) {
		BDD oneline_after = oneline_bef + con_k_tmp*encode_action(para,*i);
		this->encode_protocol_ir(para, k+1, all_case_prot,oneline_after,other_condition);
	  }
...
// default local is designed for isr/ior 
bool
basic_agent::is_valid_local_state(BDD state, vector<BDD> v)
{  
  map< string, basictype * >* e_obsvars = (*agents)[0]->get_obsvars();
  map< string, basictype * >* e_vars = (*agents)[0]->get_vars();
  if(e_obsvars!=NULL)
  	for (map< string, basictype * >::iterator i = e_obsvars->begin();
	     i != e_obsvars->end(); i++) {
	  if (!i->second->is_valid_state(state, v))
		 return false;
	}  
  if (lobsvars != NULL)
    for (map< string, variable * >::iterator i = lobsvars->begin();
         i != lobsvars->end(); i++) {
      if (!(*e_vars)[i->first]->is_valid_state(state, v))
         return false;
    }
  if (vars != NULL)
    for (map< string, basictype * >::iterator i = vars->begin();
...
bool
basic_agent::is_valid_local_state_Ir(BDD state, vector<BDD> v)
{  
  map< string, basictype * >* e_obsvars = (*agents)[0]->get_obsvars();
  map< string, basictype * >* e_vars = (*agents)[0]->get_vars();
  if(e_obsvars!=NULL)
  ...
  for(unsigned int k=1;k<agents->size();k++){
	basic_agent* this_agent = (*agents)[k];
	if (this_agent->vars != NULL)
    for (map< string, basictype * >::iterator i = this_agent->vars->begin();
         i != this_agent->vars->end(); i++) {
      if (!i->second->is_valid_state(state, v))
        return false;
    }
  }
...
string
basic_agent::seeable_state_to_str(BDD state, vector<BDD> v)
{
  ostringstream s;
  if(this->get_name()=="Environment"){
	if (obsvars != NULL)
	  for (map< string, basictype * >::iterator i = obsvars->begin();
		   i != obsvars->end(); i++) {
		s << "	  " << i->second->state_to_str(state, v);
	...
	return s.str();
  	...
  	map< string, basictype * >* en_obs = (*agents)[0]->get_obsvars();
	if(en_obs!=NULL)
	  for (map< string, basictype * >::iterator i = en_obs->begin();
				 i != en_obs->end(); i++) {
			  s << "	" << i->second->state_to_str(state, v);
	  }
    if(lobsvars!=NULL){
...
BDD
basic_agent::project_obsv_local_state(BDD * state, Cudd * bddmgr, vector<BDD> * v)
{
  BDD tmp = bddmgr->bddOne(); 
  for (int j = 0; j < (*is_agents)["Environment"]->obsvars_BDD_length(); j++) {
		tmp = tmp * (*v)[j];
  }
...
//vector< BDD >
//basic_agent::get_localstate_BDD_vector_Ir(bdd_parameters * para)
//{
//  vector<BDD> lv;
//  
//  map< string, basictype * > *obsvars = (*agents)[0]->get_obsvars(); 
...
//  for(unsigned int k=1;k<agents->size();k++){
//	basic_agent* this_agent = (*agents)[k];
//	lv.insert(lv.end(), para->v->begin()+this_agent->get_var_index_begin(), para->v->begin()+this_agent->get_var_index_end()+1);
//  }
//  return lv;  
//}
