...
{ 
  ...
  queue<BDD> queue1;
  queue<BDD> queue2;
  if((*is)==para->bddmgr->bddZero()){
   cout<<"NOTED: this init is bddZero!"<<endl;
  }
  ...
  while ((*is) != para->bddmgr->bddZero()) {
    ...
    *is = *is - curinitstate;
	...
	para->vec_reach->push_back(curinitstate);
	(*para->original_reach)+=curinitstate;
    ...
    while(!queue1.empty()) {
      ...
      while(successors != para->bddmgr->bddZero()) {
        ...
        if(k == scount) {
		  ...
		  para->vec_reach->push_back(succ);
		  (*para->original_reach)+=succ;
          queue2.push(succ);
          if(options["debug"] == 1)
...
int
SCGS_export_model(bdd_parameters * para, BDD *is, vector< vector< int >*> *countex, map< int, BDD * > *idbdd, vector<vector<transition* > > *cextr)
{ 
  int scount = 0;
  map<string, map<string, vector<BDD*>* >* >* int_vars = integer_variable_BDDs(para->bddmgr, para->v);
  queue<BDD> queue1;
  ...
  while ((*is) != para->bddmgr->bddZero()) {
    ...
    *is = *is - curinitstate;
	...
	para->vec_reach->push_back(curinitstate);
	(*para->original_reach)+=curinitstate;
	*para->num_of_inits = *para->num_of_inits + 1;
    (*idbdd)[scount++] = new BDD(curinitstate);
    if(options["debug"] == 1)
      cout << "idbdd[" << scount-1 << "] in queue1" << endl;
  }
...
print_cex(bdd_parameters * para, string fname, string str_head,  map< int, BDD * > *idbdd, vector< vector< transition * >*> *cextr)
{
  ...
  string filename ="model_export/" +fname + ".dot";
  string filename2 ="model_export/"+ fname + ".info";
...
void
SCGS_print_cex(bdd_parameters * para, string fname, string str_head,  map< int, BDD * > *idbdd, vector< vector< transition* > > *cextr)
{
  // Writing full witness/counterexample to text file
  string filename ="model_export/" +fname + ".dot";
  string filename2 ="model_export/"+ fname + ".info";
  ...
  if (myfile.is_open() && myfile2.is_open()) {
	...
	for (int ac = 0; ac < (int) cextr->size(); ac++) {
	  if(cextr->at(ac).size()==0)
		myfile << ac<<" -> size == 0"<<endl;
	  else{
		for (int j = 0; j < (int) cextr->at(ac).size(); j++) {
		  string strTran = cextr->at(ac).at(j)->index_to_string(para->a);
		  if (trans_set.find(strTran) == trans_set.end()) {
