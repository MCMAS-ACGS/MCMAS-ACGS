...
state_to_str(BDD state, vector<BDD> v)
{
  ...
  for (unsigned int i = 0; i < agents->size(); i++) {
    ...
    s << "  "<<(*agents)[i]->state_to_str(state, v)<<endl;
  ...
  for (unsigned int j = 0; j < agents->size(); j++) {
    ...
    if (vars != NULL && vars->size() > 0)
      for (map< string, basictype * >::iterator i = vars->begin();
           i != vars->end(); i++)
        if ((*i).second->get_type() == 3)//type==3 indicates that it is a enumer
          a *= ((enumerate *) (*i).second)->build_all_BDDs(bddmgr, v);//return bdd_one()
  }
  return a;
}
...
free_cudd_memory(bdd_parameters *para) 
{
  ...
  delete(para->vRT);
  delete(para->in_st);
  delete(para->num_of_inits);
  delete(para->reach);
  delete(para->original_reach);
...
void 
my_clear_cex(vector< vector< int >*>* countex, map< int, BDD * >* idbdd, 
               vector< vector< transition* > >* cextr) 
{
  for(unsigned int i=0; i<countex->size(); i++)
    delete (*countex)[i];
...
void 
my_delete_cex(vector< vector< int >*>* countex, map< int, BDD * >* idbdd, 
                vector<vector< transition* > >* cextr) 
{
