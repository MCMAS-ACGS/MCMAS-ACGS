...
modal_formula::get_operand(unsigned char i)
{
  if (i == 0 && obj[0] != NULL)
    return (obj[0]);
           ...
           (op == 1 || op == 2 || op == 4 || op == 16 || op == 17 || op == 18 || op == 19 ||
...
modal_formula::to_string()
{
  if (op == 0){
    ...
    return "A(" + ((modal_formula *) obj[0])->to_string() + " R " +
      ((modal_formula *) obj[1])->to_string() + ")";
  else if (op == 19)
    return "E(" + ((modal_formula *) obj[0])->to_string() + " R " +
      ((modal_formula *) obj[1])->to_string() + ")";
  else if (op == 30)
...
modal_formula::check_formula(bdd_parameters * para)
{
  ...
  case 0:{      // leaf
    ...
    if(options["nobddcache"] == 0)
	  ...
	  } else if (is_new_evaluation->find(name)!=is_new_evaluation->end()){
	  	result = (*is_new_evaluation)[name];
//		cout<<"this name in check_formula is: "<<name<<endl;
//		cout<<"and its result is: "<<endl;
//		global_print_state(result, para);
	  } else{
 		cout << "Error!! We cannot find proposition when checking formula!" << endl;
  ...
  case 18:{		//AR
	phi1 = ((modal_formula *) obj[0])->check_formula(para);
	phi2 = ((modal_formula *) obj[1])->check_formula(para);
    result = !(check_EU((!phi1) * (*para->reach), (!phi2) * (*para->reach), para)) * (*para->reach);
	break;
  }
  case 19:{		//ER
    phi1 = ((modal_formula *) obj[0])->check_formula(para);
	phi2 = ((modal_formula *) obj[1])->check_formula(para);
	BDD inter_result = check_EU(phi2, phi1*phi2, para);
	result = check_EG(phi2, para) + inter_result;
	break;
  ...
  case 30:{      // K
    ...
    af = !((modal_formula *) obj[1])->check_formula(para) * all_reach;
    result = all_reach - get_nK_states(&af, name, para);
    break;
  }
  ...
  case 42:{      // Distributed knowledge
    ...
    name = ((atomic_proposition *) id)->get_proposition();
    af = ((modal_formula *) obj[1])->check_formula(para);
    result = check_DK(((modal_formula *) obj[1])->check_formula(para), name, para);
    break;
  }
