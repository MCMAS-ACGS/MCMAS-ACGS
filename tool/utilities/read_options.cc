...
print_help(void)
{
  ...
  cout << "  -pg Number \t Choose parity_game based algorithm for ATL/ATL* or not for only ATL (1 -- yes, 0 --no, default 1)"  << endl;
  cout << "  -sat Number \t Choose return one feasible initial state or all feasible state (1 -- all, 0 -- one, default 1)"  << endl;
  cout << "  -print Number \t Choose if print the model file into model_export folder (1 -- yes, 0 -- no, default 0)"  << endl;
...
read_options(int argc, char *argv[]) {
  ...
  std::string ATLpg("-pg");
  std::string ATLsat("-sat");
  std::string ATLprint("-print");
  ...
  for (int i = 1; i < argc - 1; ++i) {
    ...
    } else if(ATLpg == argv[i]){
	  string s;
      s = argv[++i];    // consume the argument
	  options["ATLpg"] = atoi(s.c_str());
	  if (options["ATLpg"] < 0 || options["ATLpg"] > 1) {
        cout << "Parameter " << options["ATLpg"] <<
          " is not allowed in -pg option." << endl;
        wrongpara = true;
      } 
	} else if(ATLsat == argv[i]){
	  string s;
      s = argv[++i];    // consume the argument
	  options["ATLsat"] = atoi(s.c_str());
	  if(options["ATLsat"] < 0 || options["ATLsat"] > 1) {
		cout << "Parameter " << options["ATLsat"] <<
		  " is not allowed in -sat option." << endl;
		wrongpara = true;
	  }
	} else if(ATLprint == argv[i]){
	  string s;
      s = argv[++i];    // consume the argument
	  options["ATLprint"] = atoi(s.c_str());
	  if(options["ATLprint"] < 0 || options["ATLprint"] > 1) {
		cout << "Parameter " << options["ATLprint"] <<
		  " is not allowed in -print option." << endl;
		wrongpara = true;
	  }
	}
	else {
